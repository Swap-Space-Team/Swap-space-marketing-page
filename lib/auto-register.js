...
// The password-setup link is a credential, so we only pass one along when it points at our own app
// origin — a misconfigured or compromised backend must not be able to turn the confirmation screen
// into an open redirect. APP_URL is the SPA the link lands on, which is a different host from
// BACKEND_URL, the API this file calls; it must match the backend's own APP_URL. Fails closed:
// anything unexpected drops the link and the caller falls back to the emailed one. Never log the
// link itself.
function validateSetupLink(link) {
  ...
  try {
    ...
    if (url.origin !== new URL(APP_URL).origin) {
      console.warn(`Password-setup link origin did not match APP_URL (${APP_URL}) — dropping it`);
      return null;
    }
    return url.toString();
  } catch {
